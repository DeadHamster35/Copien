...
namespace Placien
{
  public class Main : INotifyPropertyChanged
  {
    ...
    public void Copy(string FHost, string FTarget, string FType, int FCount)
    {
      ...
      var files = GetFiles(FHost, $"*." + FType, AllDirectories);

      AppendOutput("Proceeding ...");
            
    foreach (var copytag in files)
      {
                var filename = GetFullPath(copytag);
                
                filename = filename.Substring(FCount+1);
                
        var targettag = Path.Combine(FTarget, filename);
                
        if (System.IO.File.Exists(targettag))
        {
          AppendOutput("Skipping - " + filename);
          continue;
        }

                CreateDirectory(Path.GetDirectoryName(targettag));

        System.IO.File.Copy(copytag, targettag);
      }
    ...
    {
      AppendOutput("Gathering ...");
      var files = GetFiles(Directory, "*.copytag" + Extension, AllDirectories);
